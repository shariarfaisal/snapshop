...
interface EditCourseOfferingDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  courseOffering: CourseOffering;
}

export function EditCourseOfferingDialog({ 
  open, 
  onOpenChange, 
  courseOffering 
}: EditCourseOfferingDialogProps) {
  const { toast } = useToast();
  const { programs } = useProgram();
  const { updateCourseOffering } = useCourseOffering();
  const [selectedProgramId, setSelectedProgramId] = useState<string>(courseOffering.programId);
  ...
  const form = useForm<CourseOfferingFormValues>({
    ...
    defaultValues: {
      ...
      instructorId: courseOffering.instructorId,
      academicYear: courseOffering.academicYear,
      term: courseOffering.term,
      startDate: courseOffering.startDate ? new Date(courseOffering.startDate).toISOString().split('T')[0] : "",
      endDate: courseOffering.endDate ? new Date(courseOffering.endDate).toISOString().split('T')[0] : "",
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {updateCourseOffering.isPending ? "Updating..." : "Update Course Offering"}
              </Button>
            </div>
          </form>
        </Form>
